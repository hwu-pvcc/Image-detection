...
required_libraries = ['pillow', 'torch', 'ipython']  # Replace with actual package names

# Check for missing libraries
missing = [lib for lib in required_libraries if importlib.util.find_spec(lib) is None]
...
def install_package(package_name, custom_command=None):
    ...
    try:
        if custom_command:
            subprocess.run(custom_command, check=True, shell=True)
        else:
...
if missing:
    for lib in missing:
        if lib == 'torch':
            # Handle torch installation with custom command
            success = install_package('torch', custom_command=f"{sys.executable} -m pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu")
        else:
            # Install normally
            success = install_package(lib)
        
        if not success:
            # Show an error window with Tkinter
            error = tk.Tk()
            error.attributes("-topmost", True)
            error.geometry("950x450")
            error.title("Error")
            w = tk.Label(error, text=f"Unable to install {lib}, please try again.", font=("Arial", 14))
            w.pack(pady=20)
            error.mainloop()
            sys.exit(1)
else:
